...
export default function LoginPage() {
    ...
    try {
      if (signup) {
        ...
        if (res === 201) {
          ...
          toast.error(
            'User already exists with this email. Please use a different email or try logging in.'
          );
        }
        reset();
      } else {
  ...
  const linkedInLogin = () => {
    try {
      ...
      if (!import.meta.env.VITE_LINKEDIN_CLIENT_ID) {
        toast.error(
          'LinkedIn login is not configured. Please contact support.'
        );
        return;
      }
      ...
      toast.error(
        'LinkedIn login failed. Please try again or use email/password.'
      );
    }
  };
  ...
  const facebookLogin = () => {
    try {
      ...
      toast.error(
        'Facebook login failed. Please try again or use email/password.'
      );
    }
  };
      ...
      <div className="login relative  bg-gradient-to-b from-[#CEF2FF] to-white rounded-xl shadow-lg flex flex-col items-center justify-items-end  md:py-8 md:px-10 lg:w-1/2 py-4">
        ...
        <div className="alternate ">
          ...
          <div className="google flex gap-5 md:mt-3">
            ...
            <div
              className="flex gap-2 items-center border px-3 rounded-[5px]"
